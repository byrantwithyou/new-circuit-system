...
io.on('connection', function (socket)
{
  ...
  socket.on("sendMobilePhoto", function(data) {
    let name = mobileSocket.find((element) => element.id == socket.id).name;
    let webSocketID = webSocket.find((element) => element.name == name).id;
    io.sockets.connected[webSocketID].emit("sendWebPhoto", data);   
    console.log(data[5]);
  })
  ...
  socket.on('teacherLogin',function () {
    ...
    socket.emit("authoring",subsectionServer,behaviorServer,settingServer);
  });

  socket.on("authoring",function (subsections,behaviors,settings) {
    subsectionServer = subsections;
    behaviorServer = behaviors;
    settingServer = settings;
    for(let i = 0 ; i< subsections.length; i++){
      for (let j = 0; j < subsections[i].steps.length; j++){
        stepNumber++;
      }
    }
  })
